...
**Security gate bypass for ratings:** Rating messages are internal (generated by the control plane, not user-typed). The control plane should tag them as trusted and bypass Layers 3-4 (LLM guards) of the security gate. Layers 1-2 (sanitization + heuristic) still run.

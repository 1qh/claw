...
**Cost:** Free, instant **Dependencies:** None (custom code, ~50 lines)
...
**Cost:** Free, instant, no network calls **Dependencies:** `@presidio-dev/hai-guardrails`
...
**Cost:** LLM call per message (cheap model, fast) **Dependencies:** `@presidio-dev/hai-guardrails`, `ai` (Vercel AI SDK)
...
**Cost:** Separate LLM call (Layer 3 uses hai-guardrails, Layer 4 uses AI SDK — different libraries, cannot share one call) **Dependencies:** `ai` (Vercel AI SDK)
...
**Cost:** Free (configuration only) **Dependencies:** OpenClaw native
...
**Cost:** Infrastructure cost (already part of architecture) **Dependencies:** PostgreSQL row-level security, OpenClaw per-agent path boundaries
...
Layers 1-2: <5ms (free, local) Layers 3-4: ~100-200ms (one LLM call, cheap model) Total gate overhead: ~200-400ms (two sequential LLM calls for Layers 3+4) — imperceptible to a fire-and-forget user where tasks take seconds to minutes.
